...
function startGameOxo() {
  startTimer();
  if (dataOxo.player2.username == "IA") matchIA = true;
  if (matchIA) startMatchIA();
  else startMatchPlayer();
}
...
function checkWin(tableau, player) {
  ...
  let plays = tableau.reduce((a, e, i) => (e === player ? a.concat(i) : a), []);
  let gameWon = null;
...
function endGameOxo(gameWon) {
  ...
  //   if (matchIA) getWinner(gameWon.player == oPlayer ? "Gagné!" : "Perdu!");
  //   else {
...
function getWinner(whoWin) {
  ...
  if (whoWin) {
    ...
    document.querySelector(
      ".endgame .text"
    ).innerText = `${whoWin.username} a gagné`;
  } else document.querySelector(".endgame .text").innerText = `Match nul`;
