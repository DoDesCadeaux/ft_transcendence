...
function updateProfile(element) {
  ...
  const msg_whiteSpace = document.querySelector(".whiteSpace");
  const msg_alreadyUsed = document.querySelector(".alreadyUsed");
  var error_msg = [0, 0]
  ...
  fetch(`/api/checkUsername/?username=${inputUsername}`, {
    headers: { "X-CSRFToken": csrftoken},
  })
    .then(response => response.json())
    .then((data) => {
        if (data.result){
            msg_alreadyUsed.classList.remove("displayNone");
            error_msg[0] = 1;
        }
        if (isWhiteSpace(inputUsername)){
            msg_whiteSpace.classList.remove("displayNone");
            error_msg[1] = 1;
        }
        if (error_msg[0] == 0 && error_msg[1] == 0)
            fetchToUpdate(user_id, formData);
        if (error_msg[0] == 0 && !msg_alreadyUsed.classList.contains("displayNone"))
            msg_alreadyUsed.classList.add("displayNone");
        if (error_msg[1] == 0 && !msg_whiteSpace.classList.contains("displayNone"))
            msg_whiteSpace.classList.add("displayNone");
    })
    .catch((error) => {
...
function fetchToUpdate(user_id, formData){
    fetch(`/api/update_user/${user_id}/`, {
        ...
        .then((response) => {
          (async () => {
            const data = await fetchGET(URI.USER);
            const banner_username = document.querySelector(
              ".top-banner .user .name"
            );
            const banner_photo = document.querySelector(".top-banner .user .photo");
            banner_username.textContent = data.username;
            banner_photo.src = data.photo + "?timestamp=" + new Date().getTime();
